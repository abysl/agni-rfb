...
pub static CARD: Card = unit("Sentinel Adept", &[Keyword::Weaponmaster], &[]);
...
mod tests {
    ...
    fn adept(zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Fury".into()],
            ..fixtures::unit(ADEPT, zone, seat, "Sentinel Adept", MIGHT)
        }
    }

    fn armory(equipment: bool) -> Fixture {
        let mut fixture = Fixture::enforced();
        fixture.table.cards.push(adept(fixtures::HAND, 0));
        ...
        fixture.resolve();
        assert!(std::ptr::eq(fixture.scripts.of_card(ADEPT).unwrap(), &CARD));
        fixture
    }

    fn play(ctx: &mut Ctx) {
        fixtures::play_from_hand(ctx, 0, ADEPT).unwrap();
        assert_eq!(ctx.location(ADEPT), Some(Location::Base(0)));
        assert!(
            ctx.card(ADEPT).unwrap().exhausted,
    ...
    fn the_script_is_a_weaponmaster_unit_with_the_shared_optional_play_trigger() {
        assert!(std::ptr::eq(script_of("Sentinel Adept").unwrap(), &CARD));
        assert_eq!(CARD.name, "Sentinel Adept");
    ...
    fn played_beside_an_equipment_it_offers_the_gear_and_attaches_it_for_the_equip_cost_less_a_rainbow(
        ...
        assert!(matches!(
            ...
            ItemKind::Trigger { source, index: IMPLICIT_WEAPONMASTER } if source == ADEPT
        ));
        assert!(
        ...
        assert_eq!(attached_to(&ctx, BRUTALIZER), Some(ADEPT));
        assert_eq!(
            ctx.runes_of(0).len(),
            runes - 1,
    ...
    fn skipping_the_offer_leaves_the_gear_where_it_is() {
        ...
        assert_eq!(ctx.current_might(ADEPT), i32::from(MIGHT));
    }
