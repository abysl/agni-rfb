...
pub static CARD: Card = unit("Mountain Drake", &[], &[]);
...
mod tests {
    ...
    const ENERGY: u8 = 9;
    const MIGHT: u8 = 10;
    const EXTRA_RUNES: [u32; 5] = [100, 101, 102, 103, 104];

    fn drake(id: u32, zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Body".into()],
            ..fixtures::unit(id, zone, seat, "Mountain Drake", MIGHT)
        }
    }

    fn in_hand(ready: usize) -> Fixture {
        let mut fixture = Fixture::enforced();
        fixture.table.cards.push(drake(DRAKE, fixtures::HAND, 0));
        fixture
            .table
            .cards
            .push(drake(THEIR_DRAKE, fixtures::HAND, 1));
    ...
    fn the_script_is_a_plain_unit_with_no_keywords_and_no_abilities() {
        assert_eq!(CARD.name, "Mountain Drake");
    ...
    #[test]
    fn played_from_hand_it_pays_nine_enters_the_base_exhausted_and_puts_nothing_on_the_chain() {
        let mut fixture = in_hand(ENERGY.into());
        let action = fixtures::move_action(DRAKE, fixtures::BASE, 0);
        let mut ctx = fixture.ctx_for(0, &action);
        assert_eq!(ctx.ready_runes_of(0).len(), usize::from(ENERGY));
        play_to_base(&mut ctx, 0, DRAKE).unwrap();
        assert_eq!(ctx.location(DRAKE), Some(Location::Base(0)));
        assert!(
            ctx.card(DRAKE).unwrap().exhausted,
            "a played unit enters exhausted"
        );
        assert!(
            ctx.ready_runes_of(0).is_empty(),
            "nine energy exhausts every rune"
        );
        assert!(ctx.events.iter().any(|event| matches!(
            event,
            Event::Played { card, controller: 0, origin: Origin::Hand, .. } if *card == DRAKE
        )));
        assert!(ctx.blob.chain.is_empty(), "no play trigger");
        assert!(ctx.blob.prompt.is_none(), "no optional cost to ask about");
        assert_eq!(ctx.current_might(DRAKE), i32::from(MIGHT));
        assert!(ctx.fault.is_none(), "{:?}", ctx.fault);
    }

    #[test]
    fn the_other_seat_cannot_play_it_on_this_turn_and_eight_ready_runes_are_not_enough() {
        let mut fixture = in_hand(ENERGY.into());
        let ctx = fixture.ctx();
        assert_eq!(
            legal::classify(&ctx, 1, &entry(&ctx, 1, THEIR_DRAKE)),
