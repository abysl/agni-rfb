...
pub static CARD: Card = unit("Shipyard Skulker", &[], &[]);
...
mod tests {
    ...
    const SKULKER: u32 = 90;
    const THEIR_SKULKER: u32 = 91;
    const ENERGY: u8 = 3;
    const MIGHT: u8 = 3;
    const EXTRA_RUNES: [u32; 0] = [];

    fn skulker(id: u32, zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Chaos".into()],
            ..fixtures::unit(id, zone, seat, "Shipyard Skulker", MIGHT)
        }
    }
    ...
    fn in_hand(ready: usize) -> Fixture {
        ...
        fixture
            .table
            .cards
            .push(skulker(SKULKER, fixtures::HAND, 0));
        fixture
            .table
            .cards
            .push(skulker(THEIR_SKULKER, fixtures::HAND, 1));
    ...
    fn the_script_is_a_plain_unit_with_no_keywords_and_no_abilities() {
        assert_eq!(CARD.name, "Shipyard Skulker");
        ...
        assert!(std::ptr::eq(
            fixture.scripts.of_card(SKULKER).unwrap(),
            &CARD
        ));
    }

    #[test]
    fn played_from_hand_it_pays_three_enters_the_base_exhausted_and_puts_nothing_on_the_chain() {
        let mut fixture = in_hand(ENERGY.into());
        let action = fixtures::move_action(SKULKER, fixtures::BASE, 0);
        let mut ctx = fixture.ctx_for(0, &action);
        assert_eq!(ctx.ready_runes_of(0).len(), usize::from(ENERGY));
        play_to_base(&mut ctx, 0, SKULKER).unwrap();
        assert_eq!(ctx.location(SKULKER), Some(Location::Base(0)));
        assert!(
            ctx.card(SKULKER).unwrap().exhausted,
            "a played unit enters exhausted"
        );
        assert!(
            ctx.ready_runes_of(0).is_empty(),
            "three energy exhausts every rune"
        );
        assert!(ctx.events.iter().any(|event| matches!(
            event,
            Event::Played { card, controller: 0, origin: Origin::Hand, .. } if *card == SKULKER
        )));
        assert!(ctx.blob.chain.is_empty(), "no play trigger");
        assert!(ctx.blob.prompt.is_none(), "no optional cost to ask about");
        assert_eq!(ctx.current_might(SKULKER), i32::from(MIGHT));
    ...
    fn the_other_seat_cannot_play_it_on_this_turn_and_two_ready_runes_are_not_enough() {
        ...
        let ctx = fixture.ctx();
        assert_eq!(
            legal::classify(&ctx, 1, &entry(&ctx, 1, THEIR_SKULKER)),
        ...
        assert_eq!(
            play_to_base(&mut ctx, 0, SKULKER),
