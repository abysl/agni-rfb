...
mod tests {
    ...
    fn tank(id: u32, zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            ..fixtures::unit(id, zone, seat, "Towering Combatant", MIGHT)
        }
    }
    ...
    fn contested(tank_seat: u8) -> Fixture {
        ...
        fixture
            .table
            .cards
            .push(tank(COMBATANT, fixtures::BF1, tank_seat));
        ...
        fixture.resolve();
        assert!(std::ptr::eq(
            fixture.scripts.of_card(COMBATANT).unwrap(),
            &CARD
        ));
        fixture
    }
    ...
    fn the_script_is_a_shield_two_tank_unit_with_no_abilities() {
        assert!(std::ptr::eq(
            script_of("Towering Combatant").unwrap(),
            &CARD
        ));
        assert_eq!(CARD.name, "Towering Combatant");
        ...
        let ctx = fixture.ctx();
        assert!(ctx.has_keyword(COMBATANT, Keyword::Shield(2)));
        assert!(ctx.has_keyword(COMBATANT, Keyword::Tank));
        assert_eq!(ctx.current_might(COMBATANT), i32::from(MIGHT));
        assert_eq!(
            combat::ordered(&ctx, &[BYSTANDER, COMBATANT], false),
            [COMBATANT],
    ...
    fn defending_it_shields_by_two_and_soaks_the_attackers_damage_before_its_neighbour() {
        ...
        assert!(ctx.is_defender(COMBATANT));
        assert_eq!(
            ctx.current_might(COMBATANT),
            i32::from(MIGHT + SHIELD),
            "733 · +2 while a defender"
        );
        assert_eq!(combat::lethal(&ctx, COMBATANT), MIGHT + SHIELD);
        assert_eq!(
            combat::might_sum(&ctx, &[COMBATANT, BYSTANDER]),
            MIGHT + SHIELD + BYSTANDER_MIGHT
        );
        fight(&mut ctx);
        assert_eq!(
            damage_dealt(&ctx, COMBATANT),
            i32::from(MIGHT + SHIELD),
            "the tank soaks its lethal first"
        );
        assert_eq!(ctx.card(COMBATANT).unwrap().zone, Some(fixtures::TRASH));
    ...
    fn attacking_it_has_no_shield_and_still_takes_the_defenders_damage_first() {
        ...
        let mut ctx = fixture.ctx();
        open_showdown(&mut ctx);
        assert!(ctx.is_attacker(COMBATANT));
        assert_eq!(
            ctx.current_might(COMBATANT),
            i32::from(MIGHT),
            "Shield is nothing to an attacker"
        );
        assert_eq!(combat::lethal(&ctx, COMBATANT), MIGHT);
        fight(&mut ctx);
        assert_eq!(
            damage_dealt(&ctx, COMBATANT),
            i32::from(MIGHT),
    ...
    fn the_other_seat_cannot_play_it_on_this_turn_and_a_short_seat_is_refused() {
        let mut fixture = Fixture::enforced();
        fixture.table.cards.push(tank(COMBATANT, fixtures::HAND, 0));
        ...
        assert_eq!(
            legal::classify(&ctx, 0, &entry(&ctx, 0, COMBATANT)),
        ...
        fixture.resolve();
        let action = fixtures::move_action(COMBATANT, fixtures::BASE, 0);
        let mut ctx = fixture.ctx_for(0, &action);
        legal::classify(&ctx, 0, &entry(&ctx, 0, COMBATANT)).unwrap();
        play_engine::begin(
            &mut ctx,
            0,
            COMBATANT,
        ...
        assert_eq!(ctx.location(COMBATANT), Some(Location::Base(0)));
        assert!(ctx.card(COMBATANT).unwrap().exhausted);
