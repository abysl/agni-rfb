...
pub static CARD: Card = unit("Petty Officer", &[Keyword::Assault(1)], &[]);
...
mod tests {
    ...
    const OFFICER: u32 = 90;
    const NOBODY: u32 = 91;
    const THEIR_OFFICER: u32 = 92;
    const ENERGY: u8 = 5;
    const MIGHT: u8 = 5;
    const ASSAULT: i32 = 1;
    const EXTRA_RUNES: [u32; 2] = [100, 101];

    fn officer(id: u32, zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Order".into()],
            ..fixtures::unit(id, zone, seat, "Petty Officer", MIGHT)
        }
    }
    ...
    fn attacking_into(defender_might: u8) -> Fixture {
        ...
        fixture.table.cards.push(officer(OFFICER, fixtures::BF1, 0));
    ...
    fn defending_against(attacker_might: u8) -> Fixture {
        let mut fixture = arena();
        fixture.table.cards.push(officer(OFFICER, fixtures::BF1, 1));
        fixture.table.cards.push(fixtures::unit(
            NOBODY,
            fixtures::BF1,
            0,
    ...
    fn in_hand(ready: usize) -> Fixture {
        ...
        fixture
            .table
            .cards
            .push(officer(OFFICER, fixtures::HAND, 0));
        fixture
            .table
            .cards
            .push(officer(THEIR_OFFICER, fixtures::HAND, 1));
    ...
    fn the_script_is_an_assault_one_unit_with_no_abilities() {
        assert_eq!(CARD.name, "Petty Officer");
        ...
        assert!(std::ptr::eq(
            fixture.scripts.of_card(OFFICER).unwrap(),
            &CARD
        ));
        let ctx = fixture.ctx();
        assert!(ctx.has_keyword(OFFICER, Keyword::Assault(1)));
        assert_eq!(
            ctx.current_might(OFFICER),
            i32::from(MIGHT),
    ...
    fn as_an_attacker_it_has_one_more_might_and_kills_a_defender_one_bigger_than_itself() {
        ...
        assert!(ctx.is_attacker(OFFICER));
        assert_eq!(
            ctx.current_might(OFFICER),
            i32::from(MIGHT) + ASSAULT,
            "732 · +1 might while attacking"
        );
        assert_eq!(combat::lethal(&ctx, OFFICER), MIGHT + 1);
        assert_eq!(combat::might_sum(&ctx, &[OFFICER]), MIGHT + 1);
        fight(&mut ctx);
        assert_eq!(damage_dealt(&ctx, NOBODY), i32::from(MIGHT) + ASSAULT);
        assert_eq!(
            ctx.card(NOBODY).unwrap().zone,
            Some(fixtures::TRASH),
            "six damage is lethal on six might"
        );
        assert_eq!(
            damage_dealt(&ctx, OFFICER),
            i32::from(MIGHT) + 1,
            "the bigger defender swings back for its full might"
        );
        assert_eq!(ctx.card(OFFICER).unwrap().zone, Some(fixtures::TRASH));
    }
    ...
    fn as_a_defender_the_assault_adds_nothing_and_it_trades_with_an_equal_attacker() {
        ...
        let mut ctx = fixture.ctx();
        open_showdown(&mut ctx);
        assert!(ctx.is_defender(OFFICER));
        assert_eq!(ctx.current_might(OFFICER), i32::from(MIGHT));
        assert_eq!(combat::lethal(&ctx, OFFICER), MIGHT);
        fight(&mut ctx);
        assert_eq!(damage_dealt(&ctx, OFFICER), i32::from(MIGHT));
    ...
    fn played_from_hand_it_enters_the_base_exhausted_and_the_play_is_refused_when_short() {
        ...
        let action = fixtures::move_action(OFFICER, fixtures::BASE, 0);
        let mut ctx = fixture.ctx_for(0, &action);
        play_to_base(&mut ctx, 0, OFFICER).unwrap();
        assert_eq!(ctx.location(OFFICER), Some(Location::Base(0)));
        assert!(ctx.card(OFFICER).unwrap().exhausted);
        ...
        let ctx = theirs.ctx();
        assert_eq!(
            legal::classify(&ctx, 1, &entry(&ctx, 1, THEIR_OFFICER)),
        ...
        assert_eq!(
            play_to_base(&mut ctx, 0, OFFICER),
