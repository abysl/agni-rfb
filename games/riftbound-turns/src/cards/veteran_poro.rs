...
pub static CARD: Card = unit("Veteran Poro", &[Keyword::Weaponmaster], &[]);
...
mod tests {
    ...
    fn poro(zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Body".into()],
            ..fixtures::unit(PORO, zone, seat, "Veteran Poro", MIGHT)
        }
    }

    fn armory(equipment: bool) -> Fixture {
        let mut fixture = Fixture::enforced();
        fixture.table.cards.push(poro(fixtures::HAND, 0));
    ...
    fn play(ctx: &mut Ctx) {
        fixtures::play_from_hand(ctx, 0, PORO).unwrap();
    ...
    fn the_script_is_a_weaponmaster_unit_with_the_shared_optional_play_trigger() {
        assert!(std::ptr::eq(script_of("Veteran Poro").unwrap(), &CARD));
        assert_eq!(CARD.name, "Veteran Poro");
    ...
    fn played_beside_an_equipment_it_offers_the_gear_and_attaches_it_for_the_equip_cost_less_a_rainbow(
        ...
        assert!(matches!(
            ...
            ItemKind::Trigger { source, index: IMPLICIT_WEAPONMASTER } if source == PORO
        ));
        assert!(
        ...
        assert_eq!(attached_to(&ctx, BRUTALIZER), Some(PORO));
        assert_eq!(
            ctx.runes_of(0).len(),
            runes - 1,
