...
pub static CARD: Card = with_statics(
    gear("B.F. Sword", &[Keyword::Equip(EQUIP)], &[equip(EQUIP)]),
...
mod tests {
    ...
    fn the_gear(seat: u8) -> CardInfo {
        let mut card = fixtures::gear(GEAR, fixtures::BASE, seat, "B.F. Sword", 4);
    ...
    #[test]
    fn the_script_is_a_plain_equipment_whose_whole_effect_text_is_a_might_bonus_of_3() {
        assert!(std::ptr::eq(script_of("B.F. Sword").unwrap(), &CARD));
        assert_eq!(CARD.name, "B.F. Sword");
    ...
    fn the_bonus_moves_with_the_gear_and_dies_with_the_wearer() {
        ...
        fixture
            .table
            .cards
            .push(fixtures::rune(46, 0, "Order", false));
        fixture.resolve();
        let mut ctx = fixture.ctx();
        equip_vi(&mut ctx);
        ctx.detach(GEAR);
