...
pub static CARD: Card = unit("Legion Rearguard", &[Keyword::Accelerate], &[]);
...
mod tests {
    ...
    const REARGUARD: u32 = 90;
    const THEIR_REARGUARD: u32 = 91;
    const ENERGY: u8 = 2;
    const MIGHT: u8 = 2;
    const ACCELERATED: u8 = ENERGY + 1;
    const EXTRA_RUNES: [u32; 0] = [];

    fn rearguard(id: u32, zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            ..fixtures::unit(id, zone, seat, "Legion Rearguard", MIGHT)
        }
    }
    ...
    fn in_hand(ready: usize) -> Fixture {
        ...
        fixture
            .table
            .cards
            .push(rearguard(REARGUARD, fixtures::HAND, 0));
        fixture
            .table
            .cards
            .push(rearguard(THEIR_REARGUARD, fixtures::HAND, 1));
    ...
    fn the_script_is_an_accelerate_unit_with_no_abilities() {
        assert_eq!(CARD.name, "Legion Rearguard");
        ...
        assert!(std::ptr::eq(
            fixture.scripts.of_card(REARGUARD).unwrap(),
            &CARD
        ));
        let ctx = fixture.ctx();
        assert!(ctx.has_keyword(REARGUARD, Keyword::Accelerate));
        assert!(cost::can_accelerate(&ctx, REARGUARD));
    }

    #[test]
    fn with_a_rune_to_spare_the_play_asks_to_accelerate_and_paying_one_and_a_fury_enters_ready() {
        let mut fixture = in_hand(ACCELERATED.into());
        let action = fixtures::move_action(REARGUARD, fixtures::BASE, 0);
        let mut ctx = fixture.ctx_for(0, &action);
        play_to_base(&mut ctx, 0, REARGUARD).unwrap();
        ...
        assert_eq!(
            ...
            format!("accelerate {{card {REARGUARD}}} for 1 energy and 1 Fury power?")
        );
        assert_eq!(fixtures::labels(&ctx), ["yes", "no", "cancel"]);
        assert!(ctx.effects.is_empty(), "nothing is paid before the answer");
        fixtures::choose(&mut ctx, 0, "yes").unwrap();
        assert!(ctx.blob.prompt.is_none());
        assert_eq!(ctx.location(REARGUARD), Some(Location::Base(0)));
        assert!(
            !ctx.card(REARGUARD).unwrap().exhausted,
        ...
        assert!(ctx.events.iter().any(|event| matches!(
            event,
            Event::Played { card, controller: 0, origin: Origin::Hand, .. } if *card == REARGUARD
        )));
    ...
    fn declining_the_accelerate_pays_only_the_printed_cost_and_it_enters_exhausted() {
        ...
        assert!(ctx.blob.prompt.is_none());
        assert_eq!(ctx.location(REARGUARD), Some(Location::Base(0)));
        assert!(ctx.card(REARGUARD).unwrap().exhausted);
    ...
    fn with_runes_for_the_printed_cost_only_nothing_is_asked_and_the_play_is_refused_when_short() {
        ...
        let ctx = theirs.ctx();
        assert_eq!(
            legal::classify(&ctx, 1, &entry(&ctx, 1, THEIR_REARGUARD)),
        ...
        assert_eq!(
            play_to_base(&mut ctx, 0, REARGUARD),
