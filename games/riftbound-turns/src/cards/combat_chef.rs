...
pub static CARD: Card = unit("Combat Chef", &[Keyword::Weaponmaster], &[]);
...
mod tests {
    ...
    fn chef(zone: u16, seat: u8) -> CardInfo {
        CardInfo {
            energy: Some(ENERGY),
            domain: vec!["Body".into()],
            ..fixtures::unit(CHEF, zone, seat, "Combat Chef", MIGHT)
        }
    }

    fn armory(equipment: bool) -> Fixture {
        let mut fixture = Fixture::enforced();
        fixture.table.cards.push(chef(fixtures::HAND, 0));
        ...
        fixture.resolve();
        assert!(std::ptr::eq(fixture.scripts.of_card(CHEF).unwrap(), &CARD));
        fixture
    }

    fn play(ctx: &mut Ctx) {
        fixtures::play_from_hand(ctx, 0, CHEF).unwrap();
        assert_eq!(ctx.location(CHEF), Some(Location::Base(0)));
        assert!(
            ctx.card(CHEF).unwrap().exhausted,
    ...
    fn the_script_is_a_weaponmaster_unit_with_the_shared_optional_play_trigger() {
        assert!(std::ptr::eq(script_of("Combat Chef").unwrap(), &CARD));
        assert_eq!(CARD.name, "Combat Chef");
    ...
    fn played_beside_an_equipment_it_offers_the_gear_and_attaches_it_for_the_equip_cost_less_a_rainbow(
        ...
        assert!(matches!(
            ...
            ItemKind::Trigger { source, index: IMPLICIT_WEAPONMASTER } if source == CHEF
        ));
        assert!(
        ...
        assert_eq!(attached_to(&ctx, BRUTALIZER), Some(CHEF));
        assert_eq!(
            ctx.runes_of(0).len(),
            runes - 1,
    ...
    fn skipping_the_offer_leaves_the_gear_where_it_is() {
        ...
        assert_eq!(ctx.current_might(CHEF), i32::from(MIGHT));
    }
